<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
<title> Happy Valentine bb 💗</title>

<style>
* {
  box-sizing: border-box;
  font-family: 'Segoe UI', sans-serif;
}

body {
  margin: 0;
  height: 100vh;
  background: linear-gradient(180deg, #ffdde1, #ee9ca7);
  display: flex;
  justify-content: center;
  align-items: center;
}

.phone {
  width: 100%;
  max-width: 390px;
  aspect-ratio: 9 / 16;
  background: #ffffff;
  border-radius: 28px;
  padding: 22px;
  box-shadow: 0 25px 50px rgba(0,0,0,0.25);
  display: flex;
  flex-direction: column;
  justify-content: space-between;
  overflow: hidden;
}

h1 {
  text-align: center;
  color: #ff4d6d;
  margin: 0 0 12px;
  letter-spacing: 0.5px;
}

#text {
  flex: 1;
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  font-size: 17px;
  line-height: 1.6;
  color: #333;
  padding: 10px;
  transition: opacity 0.4s ease;
}

img {
  width: 100%;
  border-radius: 18px;
  margin-top: 14px;
  display: none;
  box-shadow: 0 15px 30px rgba(0,0,0,0.25);
  animation: fadeIn 0.8s ease;
}

@keyframes fadeIn {
  from { opacity: 0; transform: scale(0.95); }
  to { opacity: 1; transform: scale(1); }
}

button {
  width: 100%;
  padding: 14px;
  border: none;
  border-radius: 14px;
  font-size: 16px;
  margin-top: 10px;
  cursor: pointer;
}

#next {
  background: #ff4d6d;
  color: #fff;
}

#no {
  background: #f2f2f2;
  color: #aaa;
  position: relative;
}

#dm {
  display: none;
  background: linear-gradient(45deg, #833ab4, #fd1d1d);
  color: white;
}

.love {
  font-size: 22px;
  line-height: 1.4;
}
</style>
</head>

<body>

<audio autoplay loop>
  <source src="https://www.bensound.com/bensound-music/bensound-love.mp3">
</audio>

<div class="phone">
  <div>
    <h1>El & Eli 💕</h1>
    <div id="text"></div>

    <!-- FOTO MUNCUL HANYA DI AKHIR -->
    <img id="photo"
      src="https://i.pinimg.com/1200x/18/6d/cc/186dcc5a78117799b7238a19ed441091.jpg"
      alt="aku & el">
  </div>

  <div>
    <button id="next">Next 💗</button>
    <button id="no">NO ❌</button>
    <button id="dm" onclick="goDM()">Balas Sekarang 💌</button>
  </div>
</div>

<script>
const texts = [
  "Hey El… 💗",
  "I made this slowly, with you on my mind.",
  "Sometimes life feels loud, but you make it quiet.",
  "Capek sama dunia itu nyata, tapi kamu selalu bikin lega. I LOVEEE UUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU",
  "Talking to you feels safe. No pressure. No pretending.",
  "Lowkey… kamu adalah bagian favorit dari hariku.",
  "I don’t need perfect moments, just moments with you.",
  "You’re soft in a way that calms me down.",
  "Kalau kamu lagi capek, please remember this:",
  "You are loved. More than you think.",
  "I choose you. Still. Every time.",
  "So… can I stay with you a little longer? pwesee? 🌷"
];

let index = 0;
const textEl = document.getElementById("text");
const photo = document.getElementById("photo");
const nextBtn = document.getElementById("next");
const noBtn = document.getElementById("no");
const dmBtn = document.getElementById("dm");

textEl.innerHTML = texts[0];

nextBtn.onclick = () => {
  index++;
  textEl.style.opacity = 0;

  setTimeout(() => {
    if (index < texts.length) {
      textEl.innerHTML = texts[index];
      textEl.style.opacity = 1;
    } else {
      nextBtn.style.display = "none";
      noBtn.style.display = "none";
      photo.style.display = "block";
      dmBtn.style.display = "block";
      textEl.innerHTML = `
        <div class="love">
          💗💗💗💗💗<br>
          Happy Valentine, El.<br>
          I hope this made you smile.
        </div>`;
      textEl.style.opacity = 1;
    }
  }, 300);
};

// tombol NO kabur
noBtn.onmouseover = () => {
  noBtn.style.transform =
    `translate(${Math.random()*80 - 40}px, ${Math.random()*40 - 20}px)`;
};

// DM Instagram + teks otomatis
function goDM() {
  const message = encodeURIComponent(
    "happy valentine el 🤍\n\n" +
    "i made something special for you.\n" +
    "hope this made you smile 💗\n\n" +
    "kalau kamu udah baca semuanya,\n" +
    "boleh yaa DM aku sekarang 🥺💌"
  );

  window.open(
    "https://www.instagram.com/direct/new/?username=drugstprn&text=" + message,
    "_blank"
  );
}
</script>

</body>
</html>
